...
export default function TakeQuiz() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ message: `Quiz submitted! Score: ${res.data.scorePercent}%`, type: "success" });
    } catch (err) {
          ...
          {quiz.questions.map((q, i) => {
            ...
            return (
              <div className="quiz-card mb-4 p-4 shadow-sm" key={i}>
                <div className="fw-semibold mb-3 text-light">Q{i + 1}. {q.question}</div>
                <div className="d-flex flex-column gap-2">
                  {!showCorrectOnly ? (
                    q.options.map((opt, idx) => {
                      ...
                      let icon = "";
                      if (result) {
                        if (optionTrimmed === correctAnswer) { highlightClass = "bg-success text-white"; icon = "✅"; }
                        else if (optionTrimmed === userAnswer) { highlightClass = "bg-danger text-white"; icon = "❌"; }
                      }
                      return (
                        <label key={idx} className={`option-label p-2 rounded ${highlightClass}`}>
                          <input type="radio" className="form-check-input me-2" name={`q${i}`}
                            checked={answers[i] === opt} disabled={!!result} onChange={() => choose(i, opt)} />
                          <span>{opt}</span>
                    ...
                    <div className="correct-answer p-2 bg-success text-white rounded">Correct Answer: {correctAnswer} ✅</div>
                  )}
                </div>
              </div>
            );
        ...
        {result && (
          <>
            <div className="alert alert-info mt-3 text-center">
              <strong>Score: {result.scorePercent}%</strong> ({result.scoreRaw}/{result.total})
            </div>
            <div className="d-flex justify-content-center gap-2 mt-3 flex-wrap">
              <button className="btn btn-outline-light" onClick={() => { setAnswers(new Array(quiz.questions.length).fill(null)); setResult(null); setShowCorrectOnly(false); }}>
                Retake Quiz
              </button>
              <button className="btn btn-outline-success" onClick={() => setShowCorrectOnly(true)}>Show Correct Answers</button>
            </div>
          </>
        )}
      </div>
