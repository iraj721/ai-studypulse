...
import { 
  ...
  FaChartLine, FaTrash, FaCheckCircle, FaTimesCircle, FaHistory 
} from "react-icons/fa";

export default function TimerPage() {
  const navigate = useNavigate();
  const { timerState, startTimer, pauseTimer, resetTimer, updateSettings, addCompletedSession, clearHistory } = useTimer();
  
  const [showSettings, setShowSettings] = useState(false);
  const [showSaveModal, setShowSaveModal] = useState(false);
  const [studyDuration, setStudyDuration] = useState(timerState.studyDuration);
  const [breakDuration, setBreakDuration] = useState(timerState.breakDuration);
  const [longBreakDuration, setLongBreakDuration] = useState(timerState.longBreakDuration);
  ...
  useEffect(() => {
    if (timerState.isActive && timerState.timeLeft > 0) {
      intervalRef.current = setInterval(() => {
        // Timer update logic would be handled by context
      }, 1000);
  ...
  const totalMinutes = timerState.sessionHistory.reduce((sum, s) => sum + s.duration, 0);
  ...
  return (
    <div className="timer-page min-vh-100 py-5 position-relative">
      ...
      <div className="container">
        ...
        {showStats && (
          <div className="stats-panel p-4 mb-4">
            <div className="stats-header">
              ...
              {timerState.sessionHistory.length > 0 && (
            ...
            <div className="stats-grid">
              <div className="stat-card">
                <div className="stat-value">{timerState.sessions}</div>
            ...
            {timerState.sessionHistory.length > 0 && (
              <div className="history-list-mini">
                ...
                {timerState.sessionHistory.slice(0, 5).map((session, idx) => (
        ...
        <div className="timer-card p-4 mb-4">
          ...
          <div className="session-info">
            <span className="session-count">✅ Completed Sessions: {timerState.sessions}</span>
        ...
        <div className="history-section">
          <h5><FaHistory /> Session History</h5>
          {timerState.sessionHistory.length === 0 ? (
            <div className="empty-history">No sessions recorded yet. Complete a study session to see it here!</div>
          ) : (
            <div className="history-grid">
              {timerState.sessionHistory.map((session) => (
                <div key={session.id} className="history-record">
                  <div className="record-icon">✅</div>
                  <div className="record-details">
                    <div className="record-title">{session.subject} - {session.topic}</div>
                    <div className="record-meta">{session.duration} min • {new Date(session.date).toLocaleString()}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {showSaveModal && (
        <div className="modal-overlay" onClick={() => setShowSaveModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <input type="text" placeholder="Topic (e.g., Algebra)" value={topic} onChange={(e) => setTopic(e.target.value)} />
      ...
      <style>{`
        .timer-page { background: linear-gradient(180deg, #080e18 0%, #122138 25%, #1e3652 50%, #28507e 75%, #1a2a3d 100%); }
        ...
        .stats-panel { background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); border-radius: 20px; color: white; }
        ...
        .history-list-mini { margin-top: 15px; }
        .history-item-mini { display: flex; justify-content: space-between; padding: 8px; background: rgba(255,255,255,0.05); border-radius: 8px; margin-bottom: 5px; font-size: 12px; flex-wrap: wrap; gap: 5px; }
        ...
        .timer-display { font-size: 80px; font-weight: bold; font-family: monospace; margin: 20px 0; }
        ...
        .history-record { display: flex; align-items: center; gap: 15px; padding: 12px; background: rgba(255,255,255,0.08); border-radius: 12px; }
        ...
        .modal-content { background: white; padding: 30px; border-radius: 24px; width: 90%; max-width: 400px; text-align: center; }
