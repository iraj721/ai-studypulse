...
exports.createActivity = async (req, res) => {
  try {
    ...
    const insightsArray = aiResponse
      .split(/\n|;/)
      .filter((line) => line.trim() !== "");
    let difficulty = "medium";
    if (/easy/i.test(aiResponse)) difficulty = "easy";
    else if (/hard|difficult|challenging/i.test(aiResponse))
      difficulty = "hard";
    ...
    // In createActivity function, make sure insight is saved with proper ID
    ...
    console.log("✅ Insight saved with ID:", savedInsight._id);
...
exports.getStats = async (req, res) => {
  try {
    ...
    const totalStudyHours = activities.reduce(
    ...
    activities.forEach((a) => {
      const diff = Math.floor(
        (today - new Date(a.createdAt)) / (1000 * 60 * 60 * 24),
      );
      if (diff >= 0 && diff < 7)
        weeklyGraph[6 - diff] += a.durationMinutes || 0;
    });
    ...
    activities.forEach((a) => {
      if (a.difficulty)
        difficultyAnalysis[a.difficulty] =
          (difficultyAnalysis[a.difficulty] || 0) + 1;
    });
...
exports.getUserInsights = async (req, res) => {
  try {
    const insights = await AIInsight.find({ user: req.user._id }).sort({
      createdAt: -1,
    });
    res.json(insights);
  } catch (err) {
